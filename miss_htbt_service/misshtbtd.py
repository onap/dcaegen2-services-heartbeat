...
def create_update_vnf_table_1(jsfile, update_db, connection_db):
    ...
    for vnf in (jhbcfg['vnfs']):
        ...
        if (nfc not in vnf_list):
            ...
            nfc, missed, intvl, clloop, policyVersion, policyName, policyScope, target_type, target, version,
            ...
            missed, intvl, clloop, policyVersion, policyName, policyScope, target_type, target, version, validity_flag,
            nfc)
        if (envPytest != 'test'):
            cur.execute(query_value)
    # heartbeat.commit_and_close_db(connection_db)
...
def main():
    try:
        ...
        i = 0
        while (True):
            hbc_pid, hbc_state, hbc_srcName, hbc_time = read_hb_common(user_name, password, ip_address, port_num,
                                                                       db_name)
            ...
            if (time_difference < 60):
                if ((int(hbc_pid) == int(pid_current)) and (source_name == hbc_srcName)):
                    ...
                    if (hbc_state == "RUNNING"):
                        state = "RUNNING"
                        update_flg = 1
                        create_update_hb_common(update_flg, pid_current, state, user_name, password, ip_address,
                                                port_num, db_name)
                ...
                if ((int(hbc_pid) != int(pid_current)) or (source_name != hbc_srcName)):
                    ...
                    if (len(job_list) >= 2):
                        ...
                        _logger.info(msg)
                        job_list = create_process(job_list, jsfile, pid_current)
                    hbc_pid, hbc_state, hbc_srcName, hbc_time = read_hb_common(user_name, password, ip_address,
                                                                               port_num, db_name)
                    update_flg = 1
                    create_update_hb_common(update_flg, pid_current, hbc_state, user_name, password, ip_address,
                                            port_num, db_name)
                else:
