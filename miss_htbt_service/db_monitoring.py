...
def send_control_loop_event(cl_type, pol_url, policy_version, policy_name, policy_scope, target_type, src_name,
                            epoc_time, closed_control_loop_name, version, target):
    msg = "DBM:Time to raise Control Loop Event for Control loop typ /target type - ", cl_type, target_type
    _logger.info(msg)
    if cl_type == "ONSET":
        ...
        if target_type == "VNF":
            json_object = json.dumps({
                ...
                "AAI": {"generic-vnf.vnf-name": src_name},
            ...
            json_object = json.dumps({
                ...
                "policyScope": policy_scope,
                "target_type": target_type,
                "AAI": {"vserver.vserver-name": src_name},
                "closedLoopAlarmStart": epoc_time,
                "closedLoopEventStatus": "ONSET",
                "closedLoopControlName": closed_control_loop_name,
                "version": version,
    ...
    while True:
        ...
        if int(current_pid) == int(hbc_pid) and source_name == hbc_src_name and hbc_state == "RUNNING":
            ...
            for event_name in vnf_list:
                ...
                db_query = "Select validity_flag,source_name_count,heartbeat_interval,heartbeat_missed_count,closed_control_loop_name,policy_version, policy_name,policy_scope, target_type,target,version from vnf_table_1 where event_name= '%s'" % (
                    event_name)
                cur.execute(db_query)
                rows = cur.fetchall()
                ...
                if validity_flag == 1:
                    for source_name_key in range(source_name_count):
                        ...
                        epoc_query = "Select last_epo_time,source_name,cl_flag from vnf_table_2 where event_name= '%s' and source_name_key=%d" % (
                        ...
                        if (epoc_time - epoc_time_sec) > comparision_time and cl_flag == 0:
                            send_control_loop_event("ONSET", pol_url, policy_version, policy_name, policy_scope,
                                                    target_type, src_name, epoc_time, closed_control_loop_name, version,
                                                    target)
                            cl_flag = 1
                            update_query = "UPDATE vnf_table_2 SET CL_FLAG=%d where EVENT_NAME ='%s' and source_name_key=%d" % (
                                cl_flag, event_name, (source_name_key + 1))
                            cur.execute(update_query)
                            connection_db.commit()
                        elif (epoc_time - epoc_time_sec) < comparision_time and cl_flag == 1:
                            send_control_loop_event("ABATED", pol_url, policy_version, policy_name, policy_scope,
                                                    target_type, src_name, epoc_time, closed_control_loop_name, version,
                                                    target)
                            cl_flag = 0
                            update_query = "UPDATE vnf_table_2 SET CL_FLAG=%d where EVENT_NAME ='%s' and source_name_key=%d" % (
